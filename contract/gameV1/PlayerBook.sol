...
contract PlayerBook {
  ...
  TeamJustInterface private constant TeamJust = TeamJustInterface(0x464904238b5CdBdCE12722A7E6014EC1C0B66928);
  ...
  function checkData(bytes32 _whatFunction) public view onlyDevs returns (bytes32, uint256) {
  ...
  function checkSignersByAddress(
    ...
    uint256 _signerB,
    uint256 _signerC
  ) public view onlyDevs returns (address, address, address) {
    return (
  ...
  function checkSignersByName(
    ...
    uint256 _signerB,
    uint256 _signerC
  ) public view onlyDevs returns (bytes32, bytes32, bytes32) {
    return (
      TeamJust.adminName(MSFun.checkSigner(msData, _whatFunction, _signerA)),
      TeamJust.adminName(MSFun.checkSigner(msData, _whatFunction, _signerB)),
      TeamJust.adminName(MSFun.checkSigner(msData, _whatFunction, _signerC))
    );
  }
  ...
  function addMeToAllGames() public isHuman {
    ...
    for (uint256 i = 1; i <= gID_; i++) {
      ...
      if (_totalNames > 1)
        for (uint256 ii = 1; ii <= _totalNames; ii++) games_[i].receivePlayerNameList(_pID, plyrNameList_[_pID][ii]);
    }
  }
  ...
  function registerNameCore(
    ...
    if (_all == true) for (uint256 i = 1; i <= gID_; i++) games_[i].receivePlayerInfo(_pID, _addr, _name, _affID);
  ...
  //  _\(/_ | |_||_)  .
  //=============|================================================================
  function addGame(address _gameAddress, string _gameNameStr) public onlyDevs {
